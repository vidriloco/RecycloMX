	...
	Map : function(gMap, opts, callback) {
		var obj = {
			initialize: function(googleMap, opts, callback) {
				...
				this.skipWritingAddressToDomAtStart = true;
			...
			setMapOptions: function(opts) {
				...
				if(opts != undefined) {
					...
					if(opts.skipWritingAddressToDomAtStart != undefined) {
						this.skipWritingAddressToDomAtStart = opts.skipWritingAddressToDomAtStart;
					}
				}
			},
			...
			writeAddressOn: function(latLng) {
				if(this.domElementForAddress && !this.skipWritingAddressToDomAtStart) {
					var geocoder = new google.maps.Geocoder();
					var instance = this;

					geocoder.geocode({'location': latLng}, function(results, status) {   
					if (typeof results[0] !== "undefined" && results[0].formatted_address.length > 0) {
							var address = results[0].formatted_address;
							$(instance.domElementForAddress).val(""+address+"");
						} 
					});
				}
				
				this.skipWritingAddressToDomAtStart = false;
			},
